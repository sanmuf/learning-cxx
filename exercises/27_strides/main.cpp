...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    //       使用逆向迭代器可能可以简化代码
    std::vector<udim>::const_reverse_iterator it=shape.rbegin();
    std::vector<udim>::reverse_iterator strides_it=strides.rbegin();
    for (; it != shape.rend(); ++it,++strides_it){
        if (it==shape.rbegin())
        {
            *strides_it=1;
        }
        else{
            *strides_it=*(strides_it-1)*(*(it-1));
        } 
    }
    return strides;
}
